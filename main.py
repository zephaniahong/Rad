...
async def get_radicale_client():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to connect to Radicale: {str(e)}")
...
async def create_calendar_event(username: str, calendar_name: str, event: CalendarEvent):
    ...
    try:
        ...
        if dtstamp.value.tzinfo is None:
            dtstamp.params['TZID'] = ['UTC']
...
async def get_calendar_events(username: str, calendar_name: str, start_date: Optional[str] = None, end_date: Optional[str] = None):
...
async def get_addressbooks():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get address books: {str(e)}")
